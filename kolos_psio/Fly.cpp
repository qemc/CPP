...
Fly::Fly(const Vector2f& position, const Texture& texture, const IntRect& choose_sprite) {
...
void Fly::chasing(const Spaceship& spaceship) {
	...
	if (is_chasing == true) {
		...
		move(fly_velocity.x * 100 * AnimatedSprite::a_time.asSeconds(), fly_velocity.y * 100 * AnimatedSprite::a_time.asSeconds());
